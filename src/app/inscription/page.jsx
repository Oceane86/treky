...
function SocialModal({ provider, onClose, onSuccess }) {
  ...
  useState(() => {
    const t1 = setTimeout(() => setStep('confirm'), 1200)
  ...
  return (
    <div className="pwd-strength">
      <div className="pwd-strength__bars">
        {[1, 2, 3, 4].map(i => (
          <span key={i} className={`pwd-strength__bar ${score >= i ? `pwd-strength__bar--${score}` : ''}`} />
        ))}
      </div>
      <div className="pwd-strength__checks">
        {checks.map(c => (
          <span key={c.label} className={`pwd-strength__check ${c.ok ? 'ok' : ''}`}>
            {c.ok ? '✓' : '○'} {c.label}
          </span>
        ))}
      </div>
      ...
      <div className="auth__panel auth__panel--form">
        ...
        <div className="auth__content">
          ...
          <div className="auth__socials">
            <button className="auth__social-btn auth__social-btn--google" onClick={() => auth.loginGoogle('/')}>
          ...
          <form className="auth__form" onSubmit={handleSubmit} noValidate>
            <div className="auth__field">
              <input type="text" placeholder="Nom complet" value={form.name} onChange={e => set('name', e.target.value)} className={`auth__input ${errors.name ? 'auth__input--error' : ''}`} />
              {errors.name && <p className="auth__error">{errors.name}</p>}
            </div>

            <div className="auth__field">
              <input type="email" placeholder="Adresse e-mail" value={form.email} onChange={e => set('email', e.target.value)} className={`auth__input ${errors.email ? 'auth__input--error' : ''}`} />
              {errors.email && <p className="auth__error">{errors.email}</p>}
            </div>

            <div className="auth__field auth__field--password">
              <input type={showPass ? 'text' : 'password'} placeholder="Mot de passe" value={form.password} onChange={e => set('password', e.target.value)} className={`auth__input ${errors.password ? 'auth__input--error' : ''}`} />
            ...
            <div className="auth__field auth__field--password">
              <input type={showConfirm ? 'text' : 'password'} placeholder="Confirmer le mot de passe" value={form.confirm} onChange={e => set('confirm', e.target.value)} className={`auth__input ${errors.confirm ? 'auth__input--error' : ''}`} />
              <button type="button" className="auth__eye" onClick={() => setShowConfirm(!showConfirm)}>
                {showConfirm ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg> : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}
              </button>
              {errors.confirm && <p className="auth__error">{errors.confirm}</p>}
            </div>

            <label className="auth__checkbox">
              <input type="checkbox" checked={form.terms} onChange={e => set('terms', e.target.checked)} />
              <span>J'accepte les <a href="#" className="auth__link">conditions d'utilisation</a> et la <a href="#" className="auth__link">politique de confidentialité</a></span>
            </label>
            {errors.terms && <p className="auth__error">{errors.terms}</p>}

            <button type="submit" className="auth__submit">S'inscrire</button>
          </form>

          <p className="auth__switch">
            Déjà inscrit ?{' '}
            <Link href="/connexion" className="auth__switch-link">Se connecter</Link>
          </p>
        </div>
      </div>
    </div>
