...
function SocialModal({ provider, onClose, onSuccess }) {
  const [step, setStep] = useState('loading') // loading | confirm | done
        ...
        {step === 'confirm' && (
          <>
            ...
            <div className="auth-modal__account">
              <div className="auth-modal__account-avatar">
                {fakeName.charAt(0)}
              </div>
              <div>
            ...
            <p className="auth-modal__info">
              Treky souhaite accéder à votre nom et adresse e-mail.
            </p>
            <div className="auth-modal__actions">
              <button className="auth-modal__btn auth-modal__btn--cancel" onClick={onClose}>
                Annuler
              </button>
              <button className={`auth-modal__btn auth-modal__btn--confirm auth-modal__btn--${provider}`} onClick={handleConfirm}>
                Autoriser
              </button>
            </div>
          </>
    ...
    <div className="pwd-strength">
      <div className="pwd-strength__bars">
        {[1, 2, 3, 4].map(i => (
          <span
            key={i}
            className={`pwd-strength__bar ${score >= i ? `pwd-strength__bar--${score}` : ''}`}
          />
        ))}
      </div>
      ...
      <div className="auth__panel auth__panel--form">
        ...
        <div className="auth__content">
          ...
          <p className="auth__switch">
            Déjà inscrit ?{' '}
            <Link to="/connexion" className="auth__switch-link">Se connecter</Link>
          </p>
        </div>
      </div>
    </div>
