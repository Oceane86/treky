...
export default function Chat() {
  ...
  const circuitName = state?.circuit?.name ?? 'votre circuit'
  ...
  function sendMessage(e) {
    ...
    // Réponse automatique simulée du guide
    setTimeout(() => {
      const replies = [
  ...
  return (
    <div className="chat">
      ...
      <div className="chat__header">
        <Link to={state?.circuit ? `/circuits/${state.circuit.slug}` : '/circuits'} className="chat__back">
          ←
        </Link>
      ...
      {state?.circuit && (
        <div className="chat__context-bar">
          <span>📋</span>
          <span>
            <strong>{state.circuit.name}</strong>
            {state.checkin && ` · Départ le ${new Date(state.checkin).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}`}
          </span>
        </div>
      )}
