...
export default function GuideSelection() {
  ...
  const availableGuides = guides.filter((g) =>
    !state.guide_ids || state.guide_ids.includes(g.id)
  )
  ...
  function chooseGuide(guide) {
    navigate('/reservation/confirmation', {
      state: {
        ...state,
        guide: { nom: guide.nom, photo: guide.photo, note: guide.note },
      },
    })
  }
    ...
    <div className="guide-sel">
      <div className="guide-sel__hero">
        ...
        <p className="guide-sel__subtitle">
          Guides disponibles du {formatDate(state.checkin)} au {formatDate(state.checkout)} · {state.circuit.name}
        </p>
      </div>
        ...
        <div className="guide-sel__grid">
          {availableGuides.map((guide) => (
            <div key={guide.id} className="guide-sel__card">
              ...
              <button
                className="btn-primary guide-sel__choose-btn"
